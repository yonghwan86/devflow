...
import { Button, Card, Input, Badge, EmptyState, Spinner, Modal, Field } from "../components/ui";
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight text-slate-800">프로젝트</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {projects.map((p) => (
            <Link key={p.id} href={`/projects/${p.id}`}>
              <Card className="group cursor-pointer transition hover:border-indigo-200 hover:shadow-md">
                ...
                <div className="mt-3"><Badge className="bg-indigo-50 text-brand">{ROLE_LABEL[p.my_role] ?? p.my_role}</Badge></div>
              </Card>
            </Link>
          ))}
        </div>
