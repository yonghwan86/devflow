...
// 관리자 설정 — AI(LLM) 설정 / 사용자 관리 2개 탭. 키는 암호화 저장, 화면에는 마스킹만.
export default function Admin() {
  const { user } = useAuth();
  const { confirm, dialog } = useConfirm();
  const [tab, setTab] = useState<"ai" | "users">("ai");
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-5">
      ...
      {/* 탭 — 사용자가 많아져도 AI 설정과 섞이지 않게 분리 */}
      <div className="flex w-fit gap-1 rounded-xl bg-slate-100 p-1 text-sm">
        <button onClick={() => setTab("ai")}
          className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-semibold transition ${tab === "ai" ? "bg-white text-brand shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          <KeyRound size={15} /> AI 설정
        </button>
        <button onClick={() => setTab("users")}
          className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-semibold transition ${tab === "users" ? "bg-white text-brand shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          <Users size={15} /> 사용자 관리 <span className="text-xs text-slate-400">{users.length}</span>
        </button>
      </div>
        ...
        <Card className="flex flex-col gap-3">
          ...
          {users.length > 8 && (
            <div className="relative">
              ...
              <Input className="pl-8" placeholder="이름 또는 이메일로 검색" value={userFilter} onChange={(e) => setUserFilter(e.target.value)} />
            </div>
          )}
          {usersQ.isLoading ? <SkeletonList count={3} lines={1} /> : (
            <div className="flex max-h-[65vh] flex-col gap-1 overflow-y-auto">
              {filteredUsers.length === 0 ? (
                <div className="py-6 text-center text-sm text-slate-400">일치하는 사용자가 없어요.</div>
              ) : filteredUsers.map((u) => {
