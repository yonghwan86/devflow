...
- **아바타 색 개편 + 가로 스크롤 ‹ › + 프리뷰 파일 불러오기(B)** — 아바타 색을 이름 해시(9색, 실팀에서도 겹침)에서 **가입 순번 기반 18색**으로 바꿔 조직 18명까지 전원 다른 색을 보장하고, 역할은 색과 분리해 **테두리**로 표시(소유자=금, 매니저=은, 툴팁에 역할). 가로로 넘치는 화면 5곳(캘린더 주간·일·타임라인, 프로젝트 탭바, 태스크 상세 탭줄)에 **‹ › 화살표** — 넘칠 때만 보이고, 클릭=한 화면·꾹 누르면 연속 이동, 표가 길어도 보이는 영역 중앙을 따라오며, 모바일에선 스와이프 가능하다는 신호 역할(탭바는 가장자리 페이드 겸용). **프리뷰**는 "파일 추가"를 "새 파일(빈 파일)"과 "**파일 불러오기**(내 PC 코드·텍스트 파일, 여러 개 선택·드래그앤드롭, 같은 이름은 덮어쓰기 확인)"로 분리. 부수: 로컬 개발 모드에서 로그인 칸에 시드 계정 자동 입력(배포 빌드엔 미포함 — 번들 검사로 확인)
