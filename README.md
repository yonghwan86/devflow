...
Claude가 DevFlow의 **실제 데이터**로 일합니다 — 태스크 조회·생성, 상태 변경, 담당자 배정, 문서 등록·분해, 가이드 작성, 지식 검색 (도구 14종).
...
| R2 MCP·설정 | 개인 API 토큰 발급/폐기 UI + MCP 연결 안내(`/settings?tab=mcp`). 도구 14종 — Claude가 설계 문서 등록→분해→배정→보고까지 수행 |
