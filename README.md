...
| S 타임라인 배율·프로젝트 기간 | 타임라인 「월 (일별)\|전체」 세그먼트 탭 — 월: 하루 28px·일 숫자 눈금·주말 음영(반복 그라데이션), 전체: % 배치로 화면 폭 압축·프로젝트 기간 연보라 띠+시작/종료 플래그·빈 곳 클릭→월 점프, localStorage 기억. 프로젝트 기간 입력 UI 신설(생성 모달·보드 헤더 인라인 수정+D-day·카드 배지) — 서버·스키마 변경 없음 |
