...
- **보드 빠른 추가 한 줄 복원** — 공용 Select의 기본 폭(100%)이 호출부 지정 폭(w-auto)을 이겨 담당자 선택이 한 줄을 통째로 먹던 충돌 수정(9곳 일괄 정상화: 보드 담당자·우선순위, 태스크 상세 담당자·선행 추가, 회의록 반영 대상, 갤러리 카테고리). 보드가 `[제목] [담당자] [+ 추가] [설명·우선순위 ▾]` 한 줄이 되고, 잘 안 보이던 "설명·우선순위 추가" 링크는 테두리 버튼으로 승격
