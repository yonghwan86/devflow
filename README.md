...
| 보안 강화(R0) | 초대 링크 계정 탈취 차단(409), MCP Bearer 전용화, CSRF 커스텀 헤더, 체크리스트 권한 축소, compose 시크릿 env 참조화 |

| 운영 역병합 | Replit 운영에서 진화한 UI 리디자인(인디고+애니메이션)·태스크 상세 탭 구조·기존 회원 직접 추가를 본 레포에 통합 |

통합 테스트 **54개**(각 Phase happy path + 권한 거부 케이스 포함) 통과, 타입체크·빌드 클린.

## UI/UX
- 인디고 brand 팔레트 + 마이크로 애니메이션/스켈레톤 로딩(운영 리디자인 반영), Plus Jakarta Sans + Pretendard
- 태스크 상세는 탭 구조(개요/체크리스트/활동/파일), 팀원은 초대 링크 또는 기존 회원 직접 추가
...
- MCP OAuth 2.1/PKCE, P11 정적 드래그드롭 호스팅, 목록 API 페이지네이션, 소프트삭제
