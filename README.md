...
| 캘린더 DnD·필터 | 주간/일 뷰에서 **카드를 드래그해 다른 요일(예정일)·팀원(담당자)으로 이동**(매니저 이상, 미배정 열 포함, 기존 REST 재사용·멱등). 범례가 **전체/할 일만/일정만 필터 버튼**으로 동작. 설정 페이지는 상위 탭 2개(**모바일 앱·알림 / MCP 연동·토큰**, `/settings?tab=mcp` 딥링크)로 분리 |
...
- 캘린더 기본 = **주간 팀원별 워크로드 그리드**(행=요일, 열=팀원), 월/일 뷰 + 팀원 필터 + 할 일/일정 필터 버튼(범례 겸용) + 카드 드래그로 요일·담당자 이동(매니저)
