...
- **잔여 이슈 일괄 해소(R4)** — 보안 4건(REST 스코프 게이트·세션 고정 방어·URL 스킴 검증·LLM base_url 검증) + **타임라인 일정 마커(◆)** + 미니 달력 할일·일정 점 표시 + 주간 뷰 일정 띠 + 야간(자정 넘김) 일정 + 티켓 승인 시 착수일 지정 + My Work 반려·무날짜 섹션 + 문서 생성 다이얼로그
