...
개발팀용 프로젝트·할일·가이드 관리 웹앱 (모바일 퍼스트 PWA). Claude가 MCP(`/api/mcp`, 도구 22종)로 실데이터를 조작하는 것이 핵심 차별점.
...
3. `README.md` — 방문자용 제품 개요·설치/배포 절차. 기능 변경 이력·개발 일지는 `CHANGELOG.md`로 분리돼 있다. 단, 개수류 수치(테스트 수 등)가 문서 간 다르면 **코드 실측이 기준**.
...
5. 문서 갱신 위치 — **이력은 `CHANGELOG.md`, README는 방문자용 개요**(세션 단위 이력을 README에 쌓지 않는다):
   - `CHANGELOG.md` → `## 최근 업데이트` 맨 위에 한 줄 + `### R1~R2 …` 표에 행 추가 (배치 코드 `(W)` 등 유지 — HANDOFF 세션과 대응)
   - 큰 기능일 때만 `README.md` → `## ✨ 주요 기능` 표에 행 추가
   - 배지·다이어그램·테스트 개수가 실제와 어긋나면 함께 수정. 전부 같은 커밋에.
