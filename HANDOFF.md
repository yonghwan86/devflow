...
## 9-S. 세션 기록 — 2026-07-14 (타임라인 배율 탭 + 프로젝트 기간 S배치)

**커밋 1개**: 타임라인 「월 (일별)|전체」 배율 탭 + 프로젝트 기간 입력 UI 3곳. 전부 클라이언트(+format.ts 헬퍼) — 서버·스키마·MCP 변경 없음.

**핵심 설계**: ① TimelineView(ProjectBoard.tsx)에 scale state("month"|"all", localStorage `devflow.timeline.scale` — 캘린더 필터와 같은 규약). **월 모드(기본)**: 연속 스크롤 캔버스 유지(막대가 월 경계에서 안 잘림 — 이전 설계 결정 존중), 하루 폭 24→28px, 주 눈금을 **일 숫자 눈금**으로 교체(토=blue-400·일=rose-400·오늘=brand 원), 주말 음영+하루 눈금선은 행마다 DOM을 늘리지 않게 트랙 div의 **반복 그라데이션 배경**(satOffset으로 요일 정렬). ② **전체 모드**: 좌표를 px 대신 **%**(posL/posW)로 — 트랙이 flex-1이라 측정 코드 없이 컨테이너 폭에 맞음. 범위 = (태스크 ∪ 프로젝트 기간) ±2일. 프로젝트 기간은 연보라 띠(bg-brand-50/70, 헤더+전 행)+시작/종료 플래그. 막대 h-2.5·칩 생략(툴팁 유지), 일정 행 숨김(events 쿼리도 enabled에서 차단), 빈 곳 클릭 → jumpTsRef에 날짜 저장 후 월 모드 전환, 스크롤 effect가 그 위치로(범위 밖이면 경계 클램프). 라벨 열은 전체 모드에서 `w-44 max-sm:w-28`. ③ **프로젝트 기간 입력 3곳**: 생성 모달 「기간 (선택)」(date 쌍, end min=start), 보드 헤더 프로젝트명 아래 표시+연필 인라인 수정(canManage, 미설정이면 매니저에게만 「기간 설정」 버튼, D-day 표시)+`PATCH /projects`(원래 지원), 프로젝트 카드 기간 배지(올해면 연도 생략 — fmtRangeShort). format.ts에 fmtDateFull(연도 포함) 추가.

**주의(다음 세션)**: 서버 projects POST/PATCH에 **start>end 역전 검증이 없다** — 클라에서 min+버튼 비활성으로 막았고, 타임라인은 역전 저장돼도 정규화해 그림(rawS/rawE 패턴과 동일). MCP엔 프로젝트 생성 도구 자체가 없어 무관. 서버 검증 추가는 선택 과제.

**미채택 결정**: 전체 모드 일정 행 축약 표시(뭉개짐 — 숨김 선택), 월 모드 하드 페이징(막대 잘림 — 연속 스크롤 유지), 타임라인 범위를 월 모드에서도 프로젝트 기간으로 확장(월 모드는 태스크 중심 유지).

**검증**: typecheck·build·테스트 77/77(서버 무변경) 통과. 프리뷰 E2E: 기간 설정 저장(API 실측 2026-06-29~10-04)+D-82, 월 모드 일 눈금 68개·오늘 pill·그라데이션 3트랙, 전체 모드 띠+플래그+막대 3·오버플로 0, 빈 곳 클릭→월 전환+스크롤 클램프, 모바일 375px 오버플로 0·라벨 112px·탭 라벨 축약, 카드 배지·생성 모달 기간 필드 DOM 실측.
...
## 9-U. 세션 기록 — 2026-07-14 (출처 문서 보기 버튼 상단 이동 U배치)

**커밋 1개**: 태스크 상세 「출처 문서 보기」를 헤더 최상단 item_key 행 우측으로 이동 + 버튼형(톤·사각) 재스타일. 클라 1파일(TaskDetail.tsx), 서버·스키마·MCP 무변경.

**배경**: 문서 파생 태스크에서 원본 문서로 돌아가는 유일한 지름길인 「출처 문서 보기」 링크가 설명 카드 밑 회색 pill(`bg-slate-100 rounded-full`)로 묻혀, 특히 넓은 웹 화면에서 발견성이 낮았다(사용자 피드백). 탭바 「문서」(ProjectNav)는 문서 목록 첫 화면으로 초기화돼 보던 문서가 유실됨.

**핵심 설계**: ① 위치 — 설명 카드 밑 → 헤더 최상단 item_key(PRJ-12) 행 우측(`ml-auto`). 새 줄 추가 없이 above-the-fold. ② 모양·색 — pill(rounded-full, 회색) → 앱 버튼 규약(`rounded-lg` 8px + brand 톤: `bg-brand-100`/`text-brand-800`/`border-brand-300`)으로 "배지(수동 태그)"가 아니라 "버튼(누르는 것)"으로 읽히게. 솔리드 '승인' 버튼보다 한 급 아래 위계 유지. ③ 문구·동작 불변 — "출처 문서 보기" 그대로, `task.source_page_id` 있을 때만 노출(문서 파생 태스크에만), 클릭 시 `?page=` 딥링크(F4 기존 동작). 휴지통 문서면 클릭 불가 회색 안내(문구 유지). ④ `font-sans` 지정(item_key 행이 font-mono라 상속 방지), `flex-wrap`로 좁은 화면 오버플로 방지.

**미채택 결정**: ① 문서 선택 URL 동기화(뒤로가기·새로고침 복원) — 모바일 PWA는 브라우저/OS 뒤로가로 대체되고 저빈도라 보류(원래 후보였으나 이 버튼 개선으로 대체). ② 문서 제목 표시(브레드크럼) — 서버 `source_page_title` 필드 필요·문구가 길어져, 고정 "출처 문서 보기"로 결정(서버 변경 회피). ③ "뒤로가기" 라벨 — history-back이 아님(캘린더 등 타 경로로 들어오면 오해). 출처 문서 링크 성격 유지.

**검증**: `npm run check`(tsc)·`npm run build`(vite) 통과. 단순 UI(클라 1파일)라 에이전트 검증·테스트 불요. **주의**: `dev:ui` 시드(dev-pglite.ts)엔 문서 파생 태스크가 없어 칩이 시드 데이터에선 안 뜬다 — 실데이터(문서 분해로 생성된 태스크)에서만 육안 확인 가능.

**배포 주의**: 스키마 변경 없음 — db:push 불필요, pull/Republish만.

## 9-V. 세션 기록 — 2026-07-14 (반응형 풀폭 — 대형 모니터 여백 해소 V배치)

**커밋 1개**: Layout `<main>`의 콘텐츠 폭 상한 제거로 큰 모니터에서 풀폭. 클라 1파일(Layout.tsx), 서버·스키마·MCP 무변경.

**배경**: 32인치 모니터에서 캘린더 주간 뷰 등 콘텐츠가 좌우로 쪼그라들고 양옆 여백이 과하게 남는다는 사용자 피드백. 원인은 열 너비가 아니라 `<main>`의 `max-w-screen-2xl`(1536px)+`mx-auto` — 모든 페이지 콘텐츠를 1536px로 가두고 가운데 정렬해, 16인치(실효폭≈상한)에선 꽉 차 보이지만 32인치(2560px)에선 양옆 ~512px씩 여백이 남았다. (**초기 오진**: 스크롤 넘침으로 착각해 열 최소너비 축소를 제안했으나, 실제는 언더필 — 폐기.)

**핵심 설계**: [Layout.tsx:118](client/src/components/Layout.tsx:118) `mx-auto w-full max-w-screen-2xl` → `w-full`. 콘텐츠가 뷰포트 폭을 그대로 사용(패딩 `px-4`/`md:px-8` 유지). 캘린더 그리드는 `minmax(16rem,1fr)`(ProjectBoard.tsx:825)이라 넓어진 폭에서 열이 `1fr`로 퍼져 자동으로 채움.

**알려진 부수효과(수용, 후속 후보)**: ① 읽기 위주 페이지(태스크 상세·문서 에디터)도 풀폭이 돼 대형 모니터에서 글줄이 길어질 수 있음 — 필요 시 해당 페이지에 읽기 폭 상한 별도 부여(현재 미적용). ② 캘린더 팀원 열이 폭에 맞춰 넓게 퍼짐(팀원 적을 때 허전하면 열 max 상한 검토, 미적용).

**검증**: `npm run check`(tsc)·`npm run build`(vite) 통과. 인앱 브라우저 스크린샷은 렌더 타임아웃으로 실패 — 32인치 실물은 사용자가 dev:ui 핫리로드로 확인. 단순 UI(클라 1파일)라 에이전트 검증·테스트 불요.
...
**후속 보강(같은 배치, 사용자 발견)**: `saveEntry` 삭제는 '저장 시점'에만 작동해 **수정 배포 전에 이미 쌓여 있던 빈 앵커**(예전에 썼다 지운 날)는 월목록에 그대로 남는 구멍이 있었다(사용자가 "7월14일 빈 기록 안 사라짐" 지적). 조회 계층에 필터 추가 — `hasRecordCond`(본문 공백 아님 OR 그 날 첨부 있음)를 월목록(`GET /`)·히트맵(`heatmapDays`)에 적용, 주간 롤업(`getRangeEntries`)은 텍스트 있는 날만(`btrim(content)<>''`). 저장 시 삭제와 별개로 조회에서 걸러 **과거 유령까지 즉시 숨긴다**(상관 서브쿼리 EXISTS). 회귀 테스트 "W: 조회 필터" 추가 → 테스트 79.

**배포 주의**: 스키마 변경 없음 — db:push 불필요, pull/Republish만.

## 9-X. 세션 기록 — 2026-07-27 (README 방문자용 재구성 + CHANGELOG 분리 X배치)

**커밋 1개**: 문서 전용(`.md` 4개). 코드·스키마 무변경.

**배경**: 사용자 지적 — "README가 보기 힘들고 업데이트 내용이 무분별하게 쌓여 있다". 실측하니 README 313줄 중 약 100줄(32%)이 세션 단위 이력이었다 — `🆕 최근 업데이트` 31항목(한 줄 300~600자) + `📜 개발 일지` 표 40행. 게다가 `(W)`·`(V)`·`(C12)` 같은 **내부 배치 코드가 그대로 노출**돼 방문자는 해독 불가인데, 정작 제품 설명은 2줄뿐이었다.

**핵심 설계**: ① 이력을 지우지 않고 **`CHANGELOG.md` 신규 파일로 전량 이관** — 업데이트 31항목·표 40행 원문 그대로(배치 코드 유지, HANDOFF 세션과 대응 관계 유지). ② README는 방문자 동선으로 재편 — 히어로(라이브 데모 CTA 크게) → "어떤 도구인가요?"(문제 제기 + `문서→할일→가이드→노하우` 핵심 루프 mermaid) → 주요 기능 표 11행(장문 불릿 대체) → Claude MCP(2단 표) → 바로 실행(가장 쉬운 `npm run dev:ui` 전면, Docker·배포·PWA는 `<details>`) → 아키텍처·보안·테스트·로드맵·더보기. 313→249줄, details 접힘 기준 **첫인상 167줄**. ③ **CLAUDE.md 워크플로 5번 동기화** — "README 최근 업데이트/개발 일지에 행 추가"가 상시 규칙이라 그대로 두면 다음 세션이 README에 이력을 다시 쌓는다 → CHANGELOG 기준으로 개정하고, 문서 읽는 순서(3번)에도 분리 사실 명시.

**부수 수정**: 테스트 개수 표기가 문서 내에서 어긋나 있었다(배지 79 vs 테스트 섹션 본문 77). "개수류는 코드 실측이 기준"(CLAUDE.md) 규칙대로 `npm test` 실행 → **79개 전부 통과** 확인 후 79로 통일.

**검증**: `.md` 전용 diff라 typecheck·build는 대상 없음(코드 파일 변경 0건 실측 확인). 대신 이 배치의 실질 위험은 **정보 손실**이므로 자동 대조로 검증 — 업데이트 항목 31→31, 표 40행→40행, 제목 누락 0건. 렌더링은 기존 의존성 `marked`로 로컬 HTML을 생성해 확인(신규 설치 없음): GitHub 호환성이 관건이던 MCP 2단 `<table>` 내부 마크다운이 정상 파싱됨(`<strong>`·`<pre><code>` 생성), mermaid 2개·표 5개·details 4개 정상.

**배포 주의**: 문서 전용 — **Replit 반영 불요**(`db:push`·Republish 모두 불필요).

**후속 필요**: `devflow-verify-push` 스킬의 4-1 단계가 아직 "README `🆕 최근 업데이트` + `📜 개발 일지`에 행 추가"로 적혀 있어 이번 개정과 어긋난다. 정본(github.com/yonghwan86/skills)에서 CHANGELOG 기준으로 고쳐 배포해야 한다.

## 9-Y. 세션 기록 — 2026-08-09 (비밀번호 찾기 — 메일 재설정 Y배치)

**배경**: 로그인 화면에 비밀번호 찾기가 없어 잊으면 들어올 방법이 없었다. 사용자가 "관리자가 링크를 보내줘야 되냐"고 되물어, 관리자 개입을 **주 경로에서 뺐다** — 관리자가 자리에 없으면 아무도 못 들어오고, 관리자 본인이 잠기면 답이 없기 때문. 메일 자가 재설정을 주 경로로 하고 관리자 발급·CLI는 **메일이 막힌 상황의 백업**으로 남겼다.

**패키지 도입**: nodemailer(승인 후 설치). Resend를 검토했으나 **도메인 인증이 필수**라 Replit 서브도메인으로는 발신 불가 — 이 한 가지가 결정적. nodemailer는 무의존성(전이 0)이고 Gmail 앱 비밀번호로 즉시 동작. `SMTP_HOST/USER/PASS`가 없으면 조용히 비활성(VAPID·LLM 키와 같은 선택 인프라 규약, `isMailEnabled()`).

**핵심 설계**: ① `password_resets`(토큰 해시 저장·2시간·`used_at` 1회용). ② `POST /auth/forgot-password` — 계정 유무와 무관하게 **같은 응답 + 같은 소요시간**(토큰을 무조건 생성해 bcrypt 시간을 균일화), 발송은 fire-and-forget(SMTP 지연이 계정 존재를 알려주지 않게). ③ `POST /auth/reset-password` — 원자적 조건부 UPDATE(`used_at IS NULL`)로 1회용 보장 → 비밀번호 교체 + 잠금 해제 → 잔여 재설정 무효화 → **세션 전량 파기 + API 토큰 폐기** → 재로그인. ④ `POST /auth/change-password`(현재 비밀번호 확인). ⑤ 관리자 발급 링크는 **세션 전용**(C5), CLI `npm run reset-password`.

**멀티에이전트 검증 확정 6건 수정**: **(A)** `/admin/users/:id/reset-link`가 Bearer 허용 → `task:write` 토큰 하나로 **완전한 계정 탈취**. C5 규약대로 세션 전용 가드. **(B)** 메일 링크를 `baseUrl(req)`로 만들어 **Host 헤더 포이즈닝** — 공격자 도메인 링크가 정품 발신자로 배달된다 → `APP_BASE_URL` 고정 + HTML 이스케이프. **(C)** 재설정이 세션만 끊고 **api_tokens는 남겨** Bearer 뒷문 존치 → 함께 폐기. **(D)** CLI가 개발용 기본 시크릿으로 무음 실패. **(E)** Admin.tsx 모바일 행 붕괴(0px). **(F)** 초대 화면에 찾기 진입점 누락.

**사용자 발견 1건**: "로그인으로 돌아가기" 버튼이 `<form>` 안에 있는데 공용 `Button`에 기본 `type`이 없어 **submit으로 동작**, 누르면 요청 접수 화면으로 튀었다. 실브라우저 조작으로만 잡힌 결함 — `type="button"` 추가 후 `<form>` 포함 3개 파일 전수 grep으로 동일 사례 없음 확인.

**환경**: 이 저장소에 dotenv가 없어(`.env`는 docker-compose 전용) 로컬에서 SMTP 값이 안 읽혔다 → npm 스크립트에 `--env-file-if-exists=.env`(Node 20.12+). 이에 맞춰 `engines.node`를 `>=20.12`로 올림.

**검증**: check·build·테스트 통과, 실제 Gmail 발송·수신·재설정까지 브라우저로 확인. **배포 시 `npm run db:push` 필요**(password_resets) + `SMTP_*`·`APP_BASE_URL` Secrets 등록.

## 9-Z. 세션 기록 — 2026-08-09 (프로젝트 보관·휴지통·영구 삭제 Z배치)

**배경**: 사용자 질문 — "전체 관리자는 프로젝트를 삭제할 수 있어야 하지 않나, 그러면 관리자에게 남의 프로젝트가 다 보이는 건가?" 확인 결과 관리자는 **목록·정리 권한만** 있고 프로젝트 내부 콘텐츠는 멤버십 기준으로 막혀 있었다(별개 축). 이어 사용자가 **"프로젝트로 얻은 노하우가 향후에 도움이 되게 설계했잖아"**라고 짚었고, 이 지적을 확인하다 내가 앞서 "노하우도 cascade로 지워진다"고 **잘못 말한 것**을 발견했다 — 실제로는 `ON DELETE SET NULL`이라 살아남는다. 그런데 살아남은 **고아 노하우가 어떤 조회에도 안 걸려** "DB에는 있는데 아무도 못 보는" 상태였다.

**설계 결정(사용자)**: 소유자에게 보관·삭제 둘 다 부여. 처음엔 "검수 전 노하우 초안이 있으면 삭제 차단"으로 정했다가, 사용자가 **"소유자가 스킬로 안 만들고 나가버렸으면 아예 못 지우는 것 아니냐"**고 지적 — 교착이 맞아 **경고 후 허용**으로 뒤집었다.

**핵심 설계**: ① `projects.deleted_at/deleted_by` 소프트 삭제(30일) + `project_deletions` 감사 스냅샷 — **projects를 참조하지 않는다**(FK면 cascade로 같이 지워져 "누가 지웠나"까지 증발). ② 라우트 `archive/trash/restore/purge` + `/trash` + `/deletion-impact`, 리터럴 `/trash`를 `/:projectId`보다 **먼저** 등록. ③ 30일 크론 + **기동 시 따라잡기**(autoscale로 자정에 자면 tick이 매일 유실돼 "다음 자정에 처리"가 성립하지 않음) + 배치 상한 50. ④ 고아 노하우 가시성을 **공용 헬퍼 하나로** 목록·상세·내보내기에 통일.

**멀티에이전트 검증(33 에이전트, 38 발견 → 28 반박검증 → 확정 14 / 기각 14)**: 확정 전건 수정. HIGH 3종 — **(A)** 파괴적 라우트 5개가 Bearer 허용: 최소 스코프 토큰 하나로 trash→purge가 200, 관리자 토큰이면 **멤버도 아닌 남의 프로젝트까지** 전멸. X배치에서 배운 C5 규약을 **바로 다음 배치에서 빠뜨린 것**. **(B)** `deleted_at` 게이트를 목록 쿼리 3개에만 걸고 `requireMember`에 안 걸어, 휴지통 프로젝트를 멤버가 계속 쓰다 30일 뒤 통째로 증발. **(C)** 고아 초안 권한 검사가 `status !== "published" && s.project_id`라 `project_id IS NULL`에서 **뒷조건 거짓 → 멤버십 검사 통째 건너뜀** → 비공개 초안 전문이 아무 로그인 사용자에게나 열림(AI 색인으로도 유출).

**미검증 low 9건 전수 재확인(사용자 요청) → 실재 9/9, 7건 수정**: **(a)** 관리자 승격 열람이 Bearer에도 적용 — 고아 가시성을 열면서 토큰 표면까지 열렸다 → 세션 전용화(본인 글은 토큰도 허용). **(b)** 자정 tick 유실 시 자동 삭제가 영영 안 돎 → 기동 시 따라잡기. **(c)** `projects_deleted_at_idx`가 SQL에만 있고 schema.ts에 선언 누락(정본 불일치). **(d)** 크론 삭제가 `purgedBy ?? p.deleted_by` 폴백 때문에 "사람이 즉시 삭제"와 같은 모양으로 기록 → 실행 주체 null(=시스템) + stats에 `auto`·`trashed_by`·원래 `project_id`(project_key는 재사용되므로 안정 식별자가 아님). **(e)** 크론 배치 무제한 → 상한 50+오래된 순+상한 도달 로그. **(f)** `deletion-impact` 실패 시 다이얼로그 자체가 안 열려 **휴지통 비우기까지 봉쇄** → 먼저 열고 요약은 나중에 채움. **(g)** 확인 입력 autoCapitalize 미차단(iOS 첫 글자 대문자화)+비활성 사유 안내 없음. **보류 2건**(범위 밖): `project_deletions` 조회 UI 부재(감사표가 사실상 write-only — 관리자 화면 신설은 별건), GET /skills마다 users 조회 1회 추가(PK 조회라 무시 가능).

**검증**: check·build·테스트 **97개** 통과(회귀 3건 신규: Bearer 차단·휴지통 읽기쓰기 차단·고아 초안 상세/내보내기 차단, 크론 감사 귀속 단언 추가). 실브라우저 e2e — 활성 프로젝트 해제·휴지통 403·복원 200·영향 집계 정확·집계 실패 주입 시에도 다이얼로그 정상.

**배포 주의**: **`npm run db:push` 필요**(projects.deleted_at·deleted_by, project_deletions, projects_deleted_at_idx).
